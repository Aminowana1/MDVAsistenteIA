...
1.4.4 keeps the optimized 1.4 routing/reliability layer but makes the normal AI path behave like the stable V1 setup again: **OpenAI `gpt-4o-mini` is primary** and Gemini is optional fallback.
...
```text
target/ServerAssistant-1.4.4.jar
```
...
1.4.4 still has no generic console execution. Plain model text is chat only. Tools must be explicitly registered in `ToolManager`, and unknown tool names are rejected. Future write/action tools should validate player permissions, arguments and current server state in Java before any action occurs.
